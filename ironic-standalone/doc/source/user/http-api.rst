...
* ``POST /v1/bmc/<bmc serial number>`` create a new bmc record or 
  update an existing bmc record.

  Response:

  * 200 - OK
  * 400 - bad request
  * 404 - record cannot be found

  Response body: JSON dictionary with keys:

  * ``gateway`` gateway of the bmc network
  * ``has_set`` (boolean) whether bmc IP address has been set
  * ``ip_address`` IP address of the bmc
  * ``netmask`` netmask of the bmc network
  * ``node_uuid`` node uuid


* ``GET /v1/bmc/<bmc serial number>`` get specified bmc record.
...
* ``GET /v1/bmcs`` list all bmc records.
  ...
  * 400 - bad request
  ...
  Response body: JSON dictionary - list of bmcrecords. Bmc record 
  representation is a JSON dictionary with keys:
  ...
  * ``netmask`` netmask of the bmc network
  * ``node_uuid`` node uuid
  * ``sn`` serial number of the bmc

* ``DELETE /v1/bmc/<bmc serial number>`` delete one bmc record by it's bmc
  serial number.
  ...
  * 404 - not found
  ...
  Request body: serial number of the deleted bmc reccord.
